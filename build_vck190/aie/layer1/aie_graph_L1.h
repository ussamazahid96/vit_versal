  ...
  public:
    ...
    simpleGraph_k1_B2_L1() {
      ...
      for (int i=0; i<L1_NUM_KERNEL;i++){
        if(i==0){
          mm[i] = kernel::create(mm0_kernel0_L1);
          source(mm[i]) = "mm0_kernel0_L1.cc";
        }
        else if(i==L1_NUM_KERNEL-1){
          mm[i] = kernel::create(mm0_kernel2_L1);
          source(mm[i]) = "mm0_kernel2_L1.cc";
        }
      ...
      for (int i=0; i<L1_NUM_KERNEL;i++){
        connect<window<L1_h1*L1_w1*4>>(in0[i], mm[i].in[0]);
        connect<window<L1_w1*L1_w2*4>>(in1[i], mm[i].in[1]);
      }

      for (int i=0; i<L1_NUM_KERNEL;i++){
        if(i==L1_NUM_KERNEL-1){
          connect<window<L1_h1*L1_w2*4>>(mm[i].out[0], out);
        }
        else{
