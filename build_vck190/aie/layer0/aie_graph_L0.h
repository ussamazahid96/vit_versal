...
class simpleGraph_k1_B4_L0: public graph {
  private:
  ...
  public:
    ...
    simpleGraph_k1_B4_L0() {
      
      for (int i=0; i<L0_NUM_KERNEL;i++){
        if(i==0){
          mm[i] = kernel::create(mm0_kernel0_L0);
          source(mm[i]) = "mm0_kernel0_L0.cc";
        }
        else if(i==L0_NUM_KERNEL-1){
          mm[i] = kernel::create(mm0_kernel2_L0);
          source(mm[i]) = "mm0_kernel2_L0.cc";
        }
        
        else{
          mm[i] = kernel::create(mm0_kernel1_L0);
          source(mm[i]) = "mm0_kernel1_L0.cc";
        }
      ...
      for (int i=0; i<L0_NUM_KERNEL;i++){
        connect<window<L0_h1*L0_w1*4>>(in0[i], mm[i].in[0]);
        connect<window<L0_w1*L0_w2*4>>(in1[i], mm[i].in[1]);
      }

      for (int i=0; i<L0_NUM_KERNEL;i++){
        if(i==L0_NUM_KERNEL-1){
          connect<window<L0_h1*L0_w2*4>>(mm[i].out[0], out);
        }
        else{
